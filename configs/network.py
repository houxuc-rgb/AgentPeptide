...
class PeptideBERT(torch.nn.Module):
    def __init__(self, bert_config, model_name):
...
def create_model(config):
    ...
    model_name = config.get('base_model_name', 'Rostlab/prot_bert_bfd')
    model = PeptideBERT(bert_config, model_name).to(config['device'])
    return model
